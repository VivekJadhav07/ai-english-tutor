...
export class App {
  ...
  async runPracticeLoop() {
    ...
    this.voice.listen(async (text, isFinal) => {
      ...
      if (isFinal && this.isTalking) {
        this.voice.stop(); // Turn off mic immediately
        ...
        try {
          ...
          // Restart the loop if user hasn't stopped the session
          if (this.isTalking) {
            this.runPracticeLoop();
          }
  ...
  private handleApiError(err: any) {
    if (err.message?.includes('429')) {
      this.isVoiceEnabled = false; // Turn off voice automatically
      this.history.push({ 
        role: 'ai', 
        text: "Voice limit reached! Switching to Text-Only mode so we can keep talking.",
        isError: true 
      });
      this.scrollToBottom();
      // Restart loop in silent mode
